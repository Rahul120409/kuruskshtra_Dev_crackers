      ...
      <header className="fixed top-0 w-full z-50 bg-surface/85 backdrop-blur-xl shadow-[0_1px_8px_rgba(0,0,0,0.25)] transition-colors duration-200">
        <div className="h-20 w-full px-margin-desktop flex items-center justify-between">
          ...
          <div className="flex items-center gap-space-lg">
            <Link href="/home" className="flex items-center gap-space-sm group">
              <img
          ...
          <div className="flex items-center gap-space-md">
            ...
            {/* Profile Avatar / Member Status */}
            <div className="flex items-center gap-space-sm pl-space-xs">
              <Link href="/profile" className="relative group">
                <img
                  alt="Profile"
                  className="w-8 h-8 rounded-full object-cover ring-1 ring-outline group-hover:scale-105 transition-transform"
                  src={user?.profileImage || 'https://lh3.googleusercontent.com/aida/AEtjO1W_VmlHNUhLRyVH2gAMu5BijoITlQYFpJefqPMLDG3RshKM-ZDs83Uzqj2j6O76_KDgGf_I_RkKx04Cint3YeRY_Tm35vZBifnl-IrkvWQak1dvtw-4OD6FCSAVmCTtSf7bm-ZKJhG0pBCtoCDvFsvrkVMONlyNUtgSWKgo-6HEWzckByicyw44xVTXagvCFoYPu-nH1hXw9qObg0WdGIAxw22u_tpi-3WgnD8LBdSxb_mwK186YSGzhP0'}
                />
                <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-secondary ring-2 ring-surface"></span>
              </Link>
              <div className="hidden lg:flex flex-col text-left">
                <span className="font-label-sm text-label-sm text-primary">VIP Client</span>
                <span className="font-body-sm text-body-sm text-on-surface font-medium truncate max-w-[120px]">
                  {user?.name ? user.name : 'Julian Sterling'}
                </span>
              </div>
            </div>
            ...
            <button
              ...
              className="xl:hidden p-space-xs rounded-lg bg-surface-container text-on-surface-variant hover:text-on-surface"
